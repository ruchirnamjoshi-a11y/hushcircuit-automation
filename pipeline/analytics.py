...
No LLM call here — this only pulls and writes the raw report. Turning it into
"what's working, what to script next" happens interactively: bring the .md
report back to a Claude Code session in this repo.
"""
...
def get_analytics_service():
    return build(ANALYTICS_API_SERVICE_NAME, ANALYTICS_API_VERSION, credentials=load_credentials())
...
def fetch_channel_metrics(start_date: date, end_date: date) -> dict:
    response = get_analytics_service().reports().query(
        ids="channel==MINE",
...
def fetch_top_videos(start_date: date, end_date: date, max_results: int = 10) -> list[dict]:
    response = get_analytics_service().reports().query(
        ids="channel==MINE",
        startDate=start_date.isoformat(),
        endDate=end_date.isoformat(),
...
def build_report(days: int = 7) -> dict:
    ...
    return {
        ...
        "channel": fetch_channel_metrics(start_date, end_date),
        "top_videos": fetch_top_videos(start_date, end_date),
    }


def _report_to_markdown(report: dict) -> str:
    lines = [
        f"# Weekly Analytics Report ({report['start_date']} to {report['end_date']})",
        "",
        "## Channel totals",
    ]
...
def write_report(report: dict, reports_dir: Path = ANALYTICS_REPORTS_DIR) -> tuple[Path, Path]:
